...
def test_MEL_insurance(client, user):
    ...
    assert InsurerInfo.objects.all().count() == 2
    assert PolicyHolderInfo.objects.all().count() == 2
    assert PolicyHolderInfo.objects.all().count() == 2
    assert PlanInfo.objects.all().count() == 2


@pytest.mark.skip(reason="For other developer create test with mel insurance.")
...
# and other developer create other test on other like functions.
